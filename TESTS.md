...
- **Mode picker** — offers Agent / Plan / Auto accept, posts `setMode` with the chosen id, closes on select, toggles closed on re-click; disabled only during the startup window (`busyLocked`) but **stays live during a running turn** so Auto accept can be picked mid-run (#64). Descriptions name the active provider. Muse's menu is Agent and Auto accept when `modeChanged.modes` says so, and the button stays hidden when an older host omits `modes` (`test/muse-mode.dom.test.ts`)
