...
| [src/voice-streamer.ts](../src/voice-streamer.ts) | Live capture (ffmpeg PCM → WebSocket STT); transcripts insert at the composer selection captured on start, manual Send/Queue discards capture and invalidates late voice callbacks, while spoken `grok send` keeps listening |
